...
import { Sparkles, Dumbbell, Droplets, Activity, Utensils, Wine, Coffee, Clock, Car, Plane, Car as CarIcon, MapPin, Waves, Users, Music, BookOpen, Landmark, FlagTriangleRight, HandHelping, Hotel, PersonStandingIcon, UsersRound } from "lucide-react";
...
export default function Amenities() {
  ...
  const getIcon = (categoryName: string, index: number) => {
    const icons = {
      core_pilgrimage: [<Landmark key={0} />, <FlagTriangleRight key={1} />, <MapPin key={2} />, <HandHelping key={3} />],
      journey_essentials: [<Car key={0} />, <Hotel key={1} />, <Plane key={2} />, <Clock key={3} />],
      cultural_immersion: [<Utensils key={0} />, <Plane key={1} />, <Waves key={2} />, <MapPin key={3} />],
      exclusive_services: [<PersonStandingIcon key={0} />, <Users key={1} />, <UsersRound key={2} />, <BookOpen key={3} />]
    };
